...
// Historical note: this file used to export requestDataDeletion() and
// getDeletionRequests() as an "intent" flow into data_deletion_requests. That
// row insert had no downstream processor, so the UI was misleading. The one
// real path (settings/data-privacy.tsx -> deleteAccount) already writes a
// data_deletion_requests audit row itself before calling admin.deleteUser, so
// nothing on the read side has been lost.
