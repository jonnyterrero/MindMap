/**
 * Report-building core, shared by the interactive server action (current user,
 * RLS client) and the scheduled cron route (any user, service-role client).
 *
 * Always scopes queries by an explicit `userId` so it is correct under either
 * client. Pure of auth/cookies — the caller supplies the client + user id.
 */
import type { SupabaseClient } from "@supabase/supabase-js";
import { computeCorrelations } from "./correlation-engine";
import { generateReportMarkdown, type ReportContext } from "./report-generator";
...
/**
 * Generate (and upsert) a report for one user. Idempotent on
 * (user_id, report_type, period_start). When `skipIfExists` is set, returns
 * `{ skipped: true }` without calling the model if a report already exists for
 * the period — important for the cron path so it doesn't re-spend tokens.
 */
export async function buildReportForUser(
  ...
  return { report: data as BuiltReport };
}
