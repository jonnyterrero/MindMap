...
2. **Expand the gold set** to ~200 dual-annotated real entries (the mandatory human spend). Release gate = **false-accept ≤ target**, graph-validity = 100%, provenance-completeness = 100% for `directly_supported`, ECE ≤ target. Edge-level gold is now SUPPORTED in the harness (`GoldEdge`; 28 claims incl. clinical/causal categories) — the 200-entry expansion should include edge annotations.
