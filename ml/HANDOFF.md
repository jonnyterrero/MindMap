...
1. ~~**Wire a real grounder in prod.**~~ DONE — `AnthropicGrounder` + `make_entailment()` (env-gated, falls back to lexical). `uv run python -m mindmap_ml.graph.evaluate --llm` runs it.
2. **Expand the gold set** to ~200 dual-annotated real entries (the mandatory human spend — still outstanding; real annotation can't be synthesised). **PARTIAL (2026-07-22):** synthetic challenge set 28 → **35 claims / 20 cases**, adding an **irrealis / non-asserted-mood family** (conditional, wish/counterfactual, future/planned, third-party attribution) + a temporal-edge positive. These exposed 4 new false-accepts, closed by a principled **assertion gate** (`evidence_scorer.assertion_gate`; see #6). Gold now: FA-rate 0.25→**0.133**, recall 1.0, ECE 0.011. Release gate for the 200-entry real set unchanged = **false-accept ≤ target**, graph-validity = 100%, provenance = 100% for `directly_supported`, ECE ≤ target; include edge annotations (`GoldEdge` supports them).
6. ~~**Assertion / mood gate**~~ DONE (2026-07-22): `evidence_scorer.assertion_gate` — a state/event node claim (`emotion`/`event`/`value`) abstains when its cited span is **irrealis** (conditional `if`/`unless`, wish, future `will`/`going to`/`plan to`, `tomorrow`/`next week`) or **attributed to a third party** (`mom/she/therapist … thinks/said/believes`, subject-adjacency check so first-person "I told …" is NOT gated). High-precision by design — `could`/`would`/`might` are deliberately excluded so "so stressed I could scream" keeps surfacing; token-level so "hopeful" ≠ "hope". Wired as gate (C1) in `verify._verify_node` before the clinical gate. Closed all 4 new adversarial FAs, recall unchanged.
3. ~~**Retrieval-evidence scorer**~~ DONE — `graph/evidence_scorer.py` (2026-07-16): clinical/diagnostic node claims must be named unhedged in the user's own text and are always capped at `weakly_inferred`; cue-less causal edges must map to a curated prior (orientation-agnostic, lemma-normalized) or they abstain. Demote/cap only — grounding never promotes. FA-rate on gold: 0.25 → 0.182 with recall 1.0 (remaining FAs = metaphor/sarcasm, the lexical grounder's known limit; the LLM grounder targets those).
4. ~~**Trained calibrator**~~ DONE — `graph/calibrate.py` (2026-07-16): Platt + isotonic (PAV), selected by leave-one-case-out Brier; artifact ships ONLY if it beats rule_v0 (it does: LOO Brier 0.107 vs 0.124 → `graph/artifacts/graph_calibrator.json`, platt a=1.211 b=1.055). Applied on top of rule_v0 in `verify._calibrate`; `MINDMAP_GRAPH_CALIBRATOR=off` forces rule_v0. In-sample gold ECE 0.108 → 0.011 (LOO table in the artifact is the honest number). **Refit + re-review the artifact whenever gold changes** (`uv run python -m mindmap_ml.graph.calibrate --write`).
5. ~~**Persistence + app surface**~~ DONE — (2026-07-22): migration 022 `mindmap_graphs` (RLS user-own SELECT only; **NO provider policy** — unlike 020 — because the payload quotes journal text verbatim and journals are user-own; applied to prod); `serving/graph_batch.py` writer (journal → pipeline → upsert on `user_id,source_table,source_id,pipeline_version`; `content_sha` skips unchanged entries; `read_journal_entries`/`read_graph_shas`/`SupabaseGraphSink` in `serving/supabase_io.py`); read-only `frontend/app/(app)/mindmap/` view (groups nodes by type, honest Stated/Inferred badges, verified connections) + "Mindmap" nav entry; daily cron `.github/workflows/ml-graph-cron.yml` (inert until secrets; `--llm` when ANTHROPIC_API_KEY present, else rule-skeleton). 12 batch tests; full ml suite green; frontend typecheck+build green.

### Remaining work (post-#5)
- **#2 gold expansion** (above) is now the highest-value ML item.
- **Cron secrets**: `ml-graph-cron.yml` is inert until `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` (+ optional `ANTHROPIC_API_KEY`) are set as repo Actions secrets. Until then no graphs are written and the app's Mindmap view shows its empty state. **Rotate the keys pasted in chat earlier before using them as the secret.**
- **Graph quality in prod** depends on the LLM extractor: without `ANTHROPIC_API_KEY` the batch writes rule-skeleton graphs (one node per sentence), not the multi-node concept graph.
...
`cd ml && uv run pytest` to confirm green, then `uv run python -m mindmap_ml.graph.evaluate` to see the verifier state (FA 0.182 / recall 1.0 / ECE 0.011 on the lexical grounder as of 2026-07-16). Items #1/#3/#4/#5 are done — the pipeline is now wired end to end (journal → verified graph → `mindmap_graphs` → app Mindmap view). Next: **#2 gold expansion** (unlocks trusting absolute numbers + a real release gate), and set the cron secrets so graphs actually get written in prod.
