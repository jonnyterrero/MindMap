...
- `28fe3ca` **wire-up** — write path encrypts (`journal/actions.ts` create + update, `sync-actions.ts`, `voice-actions.ts`), read path decrypts (`getJournalEntries`, `reflectOnJournalEntry`, `/api/export`, `settings/data-privacy-actions.ts::exportUserData`), backfill script at `frontend/scripts/backfill-journal-encryption.ts`, `/privacy` §9 rewritten to match reality.

Every wired site guards on `isEncryptionEnabled()`, so with `JOURNAL_ENCRYPTION_MASTER_KEY` **unset** in Vercel, main behaves identically to before the merge. The cutover is a single env-var set + one backfill run.

## Cutover checklist (do in the cloud, in this order)

1. **Apply migration 027** to production (Supabase MCP `apply_migration` OR paste `supabase/migrations/027_journal_encryption_user_keys.sql` into SQL Editor). Idempotent-ish — the table is `CREATE TABLE IF NOT EXISTS`. New table, no data risk.
2. **Generate a master key locally:** `openssl rand -hex 32`. Keep a copy somewhere durable (a password manager works). Losing it = journals permanently unreadable.
3. **Set `JOURNAL_ENCRYPTION_MASTER_KEY`** in Vercel Production + Preview + Development. Vercel will trigger a redeploy.
4. **Verify a new journal entry encrypts:** log in on the preview URL, create a journal entry, run `select id, content, encryption_algo, length(body_encrypted) from mindmap_journal_entries order by created_at desc limit 1;` — expect `content=NULL`, `encryption_algo='aes-256-gcm'`, `length(body_encrypted) >= 28`.
5. **Verify decryption round-trip:** reload `/journal`. The entry should render normally. Also confirm `/settings` "Export my data" downloads a JSON bundle with plaintext journal `content` and null ciphertext columns.
6. **Backfill historical rows** (only after step 5 confirms the read path works): from the repo, with the same env var set + `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`:
   ```
   node --import ./scripts/ts-resolve-hook.mjs frontend/scripts/backfill-journal-encryption.ts          # dry-run
   node --import ./scripts/ts-resolve-hook.mjs frontend/scripts/backfill-journal-encryption.ts --apply  # writes
   ```
   Re-run `--apply` any time; it only touches rows still at `encryption_algo='none'`.

## Known follow-ups (deliberately deferred)

- **ML pipeline decrypt** — `ml/mindmap_ml/serving/supabase_io.py::read_journal_entries` already filters out rows with null `content`, so the graph pipeline silently *skips* encrypted entries. Not a crash, but the graph loses coverage of every entry written after cutover. Follow-up: an ML-side decrypt using the same wrap/unwrap protocol (needs the master key in the GH Actions env for the cron; the wrap-row schema is stable so no migration).
- **`mindmap_voice_notes.transcript`** — voice notes' journal body is now encrypted, but the SAME text is still stored plaintext in `mindmap_voice_notes.transcript`. Out of scope per ADR-001; needs its own ADR + a matching migration.
- **DEK rotation** — no CLI yet. The wrap row already has `wrap_master_key_version` so a rotation PR just adds `JOURNAL_ENCRYPTION_MASTER_KEY_V2`, a new active DEK per user, and re-writes journal `encryption_key_id` — no schema change.
- **Local scaffold branch** — `feat/journal-encryption-scaffold` still exists locally on the Windows machine (the rebase left it dangling; git refused `-d` because the old hashes are no longer strict ancestors of main). Remote branch already deleted. When you next work locally: `git branch -D feat/journal-encryption-scaffold`. Content is verified on main under `6fd1580` + `28fe3ca`.

## Still open from prior sessions (unchanged this session)

Everything in the "Still open" lists above still applies:

- 0.1 key rotation, 1.5 encryption *decision* (now DONE — ADR-001 accepted + shipped), Playwright E2E, `recharts` 2→3, the two Supabase advisor WARNs, App Store TestFlight prep on a Mac.

## The user's local in-flight work — NOT touched this session

- `docs/app-store-checklist.md` (+3 lines uncommitted, "Parked 2026-09-09: no Mac available")
- `frontend/MOBILE.md` (+3 lines uncommitted, same parking note)
- `frontend/AGENTS.md`, `frontend/CLAUDE.md` (Next.js agent-rules stubs auto-generated by `next dev` — worth committing; the AGENTS.md content warns "This is NOT the Next.js you know")
- `docs/adr/` (ADR-001 draft, already Accepted 2026-09-10 — implementation shipped this session)

Nothing session 4 touched overlaps any of these.

## Session 4 verification recap

- `pnpm typecheck` 0
- `pnpm build` 0
- `pnpm test` 55/55 pass, 1 pre-existing skipped (Node's built-in runner, `node --test`)
- 15 crypto tests: round-trip, wrong-key rejects, GCM auth trips on bit-flip, empty-string + unicode edges, IV randomness, wrong-length inputs rejected, secret never appears in error messages
- Fast-forward-then-rebase against `origin/main` (2 DEV_PLAN doc commits landed between session start and push) — clean, zero file overlap
